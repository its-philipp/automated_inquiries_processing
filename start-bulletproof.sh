...
# Start each port forward individually and wait for it to establish
kubectl port-forward -n inquiries-system svc/streamlit-dashboard 8501:8501 > /dev/null 2>&1 &
sleep 2
kubectl port-forward -n monitoring svc/prometheus-grafana 3000:80 > /dev/null 2>&1 &
sleep 2
kubectl port-forward -n airflow svc/airflow-webserver 8080:8080 > /dev/null 2>&1 &
sleep 2
kubectl port-forward -n argocd svc/argocd-server 30009:443 > /dev/null 2>&1 &
sleep 3
